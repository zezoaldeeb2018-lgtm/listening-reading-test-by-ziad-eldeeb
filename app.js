...
function toggleForm(){ document.getElementById('form-group').style.display=document.getElementById('type-select').value==='listening'?'block':'none'; }
function startExam(){ setExamStudentName(); currentSkill=document.getElementById('type-select').value; selectedForm=document.getElementById('form-select').value; document.getElementById('info-badge').innerText=`[Level ${selectedLevel}${currentSkill==='listening'?' - نموذج '+selectedForm:''} - ${currentPhone}]`; if(currentSkill==='listening') currentQuestions=levelsData[selectedLevel].listening[selectedForm]; else currentQuestions=levelsData[selectedLevel].reading; userAnswers=new Array(currentQuestions.length).fill(undefined); switchScreen('exam-screen'); if(currentSkill==='listening'){ setupListeningUI(); } else { setupReadingUI(); } playQuestion(0); }
...
function playQuestion(index){ if(index>=currentQuestions.length){ showResults(); return; } currentIndex=index; const q=currentQuestions[index]; document.getElementById('part-title').innerText=q.partName||getTagTitle(q.tag); document.getElementById('speaker-name').innerText=`${q.speaker||'سؤال '+(index+1)} من ${currentQuestions.length}`; document.getElementById('question-text').innerText=q.question; document.getElementById('timer-text').innerText=""; const expBox=document.getElementById('explanation-box'); expBox.style.display='none'; const box=document.getElementById('options-box'); box.innerHTML=''; q.options.forEach((opt,i)=>{ const btn=document.createElement('button'); btn.className='opt-btn'; btn.innerText=opt; if(userAnswers[currentIndex]!==undefined){ if(currentSkill==='reading'){ if(i===q.correct) btn.classList.add('correct'); if(userAnswers[currentIndex]===i&&i!==q.correct) btn.classList.add('wrong'); btn.disabled=true; } else if(userAnswers[currentIndex]===i) btn.classList.add('selected'); } btn.onclick=()=>selectOption(i); box.appendChild(btn); }); if(currentSkill==='reading'&&userAnswers[currentIndex]!==undefined) showExplanation(q); if(currentSkill==='listening'){ if(!audio.src.includes(encodeURI(q.audioSrc))){ audio.src=q.audioSrc; } audio.currentTime=q.startTime; audio.play().catch(e=>console.log("autoplay",e)); } }
