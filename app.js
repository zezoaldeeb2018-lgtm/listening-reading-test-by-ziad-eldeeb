...
function playQuestion(index){ if(index>=currentQuestions.length){ showResults(); return; } currentIndex=index; const q=currentQuestions[index]; document.getElementById('part-title').innerText=q.partName||getTagTitle(q.tag); document.getElementById('speaker-name').innerText=`${q.speaker||'سؤال '+(index+1)} من ${currentQuestions.length}`; document.getElementById('question-text').innerHTML=`<bdi dir="ltr" style="unicode-bidi:isolate;text-align:left;display:inline-block;width:100%;">${q.question}</bdi>`; document.getElementById('timer-text').innerText=""; const expBox=document.getElementById('explanation-box'); expBox.style.display='none'; const box=document.getElementById('options-box'); box.innerHTML=''; q.options.forEach((opt,i)=>{ const btn=document.createElement('button'); btn.className='opt-btn'; btn.innerHTML=`<bdi dir="ltr" style="unicode-bidi:isolate;text-align:left;display:inline-block;width:100%;">${opt}</bdi>`; if(userAnswers[currentIndex]!==undefined){ if(currentSkill==='reading'){ if(i===q.correct) btn.classList.add('correct'); if(userAnswers[currentIndex]===i&&i!==q.correct) btn.classList.add('wrong'); btn.disabled=true; } else if(userAnswers[currentIndex]===i) btn.classList.add('selected'); } btn.onclick=()=>selectOption(i); box.appendChild(btn); }); if(currentSkill==='reading'&&userAnswers[currentIndex]!==undefined) showExplanation(q); if(currentSkill==='listening'){ if(!audio.src.includes(encodeURI(q.audioSrc))){ audio.src=q.audioSrc; } audio.currentTime=q.startTime; audio.play().catch(e=>console.log("autoplay",e)); } }
